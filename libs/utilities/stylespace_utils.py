...
def conv_warper(layer, input, style, noise, only_modulate = False):
	# the conv should change
	if not only_modulate:
		conv = layer.conv
	else:
		conv = layer
...
def rgb_warper(layer, input, style, skip = None):
	out = conv_warper(layer.conv, input, style, None, True)
	out = out + layer.bias
...
def decoder_with_rgb(G, style_space, rgb_space, noise, resize_image = True, return_with_16x16 = False):
	# an decoder warper for G
	out = G.input(style_space[0])
	out = conv_warper(G.conv1, out, style_space[0], noise[0])
	skip = rgb_warper(G.to_rgb1, out, rgb_space[0])
	...
	for conv1, conv2, noise1, noise2, to_rgb in zip(
		...
		i += 2
		skip = rgb_warper(to_rgb, out, rgb_space[j], skip)	
		j += 1
		if i == 9:
			features_16x16 = out
	...
	if return_with_16x16:
		return image, features_16x16
	else:
		return image
...
def decoder_from_16x16(G, features_16x16, fuser, style_space, rgb_space, noise, resize_image = True):
	# an decoder warper for G
	out = G.input(style_space[0])
	out = conv_warper(G.conv1, out, style_space[0], noise[0])
	skip = rgb_warper(G.to_rgb1, out, rgb_space[0])
	...
	for conv1, conv2, noise1, noise2, to_rgb in zip(
		...
		if i == 9:
			out = fuser(torch.cat((features_16x16, out), 1))
