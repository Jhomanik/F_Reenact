...
class Encoder4StyleEditing(Encoder4Editing):
    def __init__(self, num_layers, mode='ir', image_resolution = 256, style_dim = 512, channel_multiplier = 1):
        Encoder4Editing.__init__(self, num_layers, mode, image_resolution)
        self.channels = {
            4: 512,
            8: 512,
            16: 512,
        ...
        self.to_style1 = GradualStyleBlock(512, self.channels[4], 16)
        self.to_rgb1 = GradualStyleBlock(512, self.channels[4], 16)
        self.to_styles = nn.ModuleList() 
        self.to_rgbs = nn.ModuleList()
        self.log_size = int(math.log(image_resolution, 2))
        
        in_channel = self.channels[4]
        for i, c in enumerate(range(3, self.log_size + 1)):
            out_channel = self.channels[2 ** c]
            if 2*i < self.coarse_ind:
                spacial = 16
            elif 2*i < self.middle_ind:
                spacial = 32
            else:
                spacial = 64

            self.to_styles.append(GradualStyleBlock(512, in_channel, spacial))

            self.to_styles.append(GradualStyleBlock(512, out_channel, spacial))

            self.to_rgbs.append(GradualStyleBlock(512, out_channel, spacial))

            in_channel = out_channel
    

    
    def return_16x16_features(self, x):
        x = self.input_layer(x)
    ...
    def forward(self, x, mean_styles = None, mean_rgbs = None):
        x = self.input_layer(x)
        ...
        final_styles.append(self.to_style1(c3))
        final_rgbs.append(self.to_rgb1(c3))
       

        features = c3
        for i in range(len(self.to_rgbs)):  # Infer additional deltas
            if 2*i-1 == self.coarse_ind:
                
                p2 = _upsample_add(c3, self.latlayer1(c2))  # FPN's middle features
                features = p2
            elif 2*i-1 == self.middle_ind:
                
                p1 = _upsample_add(p2, self.latlayer2(c1))  # FPN's fine features
                features = p1
            
            final_styles.append(self.to_styles[2*i](features))
            final_styles.append(self.to_styles[2*i+1](features))
            final_rgbs.append(self.to_rgbs[i](features))
        if mean_styles is not None and mean_rgbs is not None:
            final_styles = [x + y.detach() for x, y in zip(final_styles, mean_styles)]
            final_rgbs = [x + y.detach() for x, y in zip(final_rgbs, mean_rgbs)]
        return final_styles, final_rgbs
